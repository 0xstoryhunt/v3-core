...
import { TestStoryHuntV3SwapPay } from '../typechain/TestStoryHuntV3SwapPay'
...
import { TestStoryHuntV3ReentrantCallee } from '../typechain/TestStoryHuntV3ReentrantCallee'
...
describe('StoryHuntV3Pool', () => {
  ...
  let factory: StoryHuntV3Factory
  let pool: MockTimeStoryHuntV3Pool

  let swapTarget: TestStoryHuntV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeStoryHuntV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestStoryHuntV3ReentrantCallee')
      ).deploy()) as TestStoryHuntV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestStoryHuntV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestStoryHuntV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestStoryHuntV3SwapPay
